 ...
 */
xn.PagingStore = Ext.extend(xn.Store, {
	xnIsClientPaged: true,
	...
	loadData: function (o, append, params) {
		// SHOULD: check the store's proxy, reader, and whatnot to see when xaction is needed. Phew, this ExtJS data store design is overly complex for little benefit.
		var p = Ext.apply({ xaction: 'read' }, params || (this.lastOptions ? this.lastOptions.params : this.baseParams));
		this.isPaging(p);
	...
	getTotalCount: function () {
		return this.allData ? this.allData.getCount() : this.totalLength || this.data.getCount() || 0;
	},

	sortData: function (f, direction, silent) {
		var f = this.sortInfo.field;
		var direction = this.sortInfo.direction;
	...
	filterBy: function (fn, scope, silent) {
	...
	xnForceLoad: function (options) {
		delete this.lastParams;
		this.load(options || this.lastOptions);
	},
...
xn.PagingDirectStore = function(c) {
	c.batchTransactions = false;
	xn.PagingDirectStore.superclass.constructor.call(this, Ext.apply(c, {
...
Ext.extend(xn.PagingDirectStore, xn.PagingStore, {});
Ext.reg('pagingdirectstore', xn.PagingDirectStore);

xn.PagingJsonStore = Ext.extend(xn.PagingStore, {
	constructor: function(config) {
		xn.PagingJsonStore.superclass.constructor.call(this, Ext.apply(config, {
			reader: new Ext.data.JsonReader(config)
		}));
	}
});
Ext.reg('pagingjsonstore', xn.PagingJsonStore);

xn.PagingXmlStore = Ext.extend(xn.PagingStore, {
	constructor: function(config) {
		xn.PagingXmlStore.superclass.constructor.call(this, Ext.apply(config, {
			reader: new Ext.data.XmlReader(config)
		}));
	}
});
Ext.reg('pagingxmlstore', xn.PagingXmlStore);


xn.PagingArrayStore = Ext.extend(xn.PagingStore, {
	constructor: function(config) {
		xn.PagingArrayStore.superclass.constructor.call(this, Ext.apply(config, {
	...
	loadData: function(data, append) {
		...
		xn.PagingArrayStore.superclass.loadData.call(this, data, append);
	}
});
Ext.reg('pagingarraystore', xn.PagingArrayStore);

xn.PagingSimpleStore = xn.PagingArrayStore;
Ext.reg('pagingsimplestore', xn.PagingSimpleStore);
